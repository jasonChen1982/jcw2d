...
RenderTexture.prototype.renderWebGL = function(displayObject, matrix, clear, updateTransform) {
  ...
  let temp = this.renderer.filterManager;
...
RenderTexture.prototype.renderCanvas = function(displayObject, matrix, clear, updateTransform) {
  ...
  let cachedWt = displayObject.worldTransform;
  ...
  //    this.textureBuffer.
  let context = this.textureBuffer.context;

  let realResolution = this.renderer.resolution;
...
RenderTexture.prototype.getCanvas = function() {
  if (this.renderer.type === CONST.RENDERER_TYPE.WEBGL) {
    let gl = this.renderer.gl;
    let width = this.textureBuffer.size.width;
    let height = this.textureBuffer.size.height;
    ...
    let tempCanvas = new CanvasBuffer(width, height);
    let canvasData = tempCanvas.context.getImageData(0, 0, width, height);
