...
Transition.prototype.update = function(snippet) {
  ...
  if (this.progress < this.duration) {
    ...
    if (this.repeats > 0 || this.infinity) {
      ...
      this.progress = 0;
      if (this.alternate) {
        let sc = JC.UTILS.copyJSON(this.ATRS);
...
Animation.prototype.configKey = function() {
  ...
  let config = this._keyConfig[Math.min(this._keyIndex, this._keyIndex - this._direction)] || {};
...
MovieClip.prototype.getFramePos = function() {
  ...
  if (cf > 0) {
    let row = this.element._textureW / this.element.width >> 0;
    let lintRow = this.sx / this.element.width >> 0;
    let lintCol = this.sy / this.element.height >> 0;
    let mCol = lintCol + (lintRow + cf) / row >> 0;
...
PathMotion.prototype.nextPose = function() {
  ...
  //     if(this.element[i]!==undefined)this.element[i] = cache[i];
  // }
  let t = JC.TWEEN[this.ease](this.progress, 0, 1, this.duration);
  let pos = this.getPoint(t, this.points);
  cache.x = pos.x;
  cache.y = pos.y;
  // cache.y = pos.y;
...
PathMotion.prototype.decomposeRotate = function(t) {
  let p1 = this.getPoint(t, this.points);
  let p2 = this.getPoint(t + 0.01, this.points);
  let deg = p1.xxxxx + p2.x;
};
...
Animator.prototype.fromTo = function(opts) {
  let animate = new JC.Transition(opts);
  this.animates.push(animate);
  return animate;
};
Animator.prototype.keyFrames = function(opts) {
  let animate = new JC.Animation(opts);
  this.animates.push(animate);
  return animate;
};
