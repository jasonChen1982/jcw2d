 ...
 * @param {object} [uniforms] Uniforms for this shader.
 * @param {object} [attributes] Attributes for this shader.
 */
function Shader(shaderManager, vertexSrc, fragmentSrc, uniforms, attributes) {
  if (!vertexSrc || !fragmentSrc) {
    throw new Error('js Error. Shader requires vertexSrc and fragmentSrc');
  }
 ...
 *
 * @param {string} keys the uniforms to cache
 */
 ...
 *
 * @param {string} keys the attributes to cache
 */
...
Shader.prototype.compile = function() {
  ...
  const glVertShader = this._glCompile(gl.VERTEX_SHADER, this.vertexSrc);
  const glFragShader = this._glCompile(gl.FRAGMENT_SHADER, this.fragmentSrc);
  ...
  if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
    console.error('js Error: Could not initialize shader.');
    console.error('gl.VALIDATE_STATUS', gl.getProgramParameter(program, gl.VALIDATE_STATUS));
    console.error('gl.getError()', gl.getError());

    // if there is a program info log, log it
    if (gl.getProgramInfoLog(program) !== '') {
      console.warn('js Warning: gl.getProgramInfoLog()', gl.getProgramInfoLog(program));
    }
 ...
 * Adds a new uniform
 *
 * @param {object} uniform the new uniform to attach
 */
Shader.prototype.syncUniform = function(uniform) {
  ...
  default:
    console.warn('js Shader Warning: Unknown uniform type: ' + uniform.type);
  }
};
...
Shader.prototype.initSampler2D = function(uniform) {
  ...
  const texture = uniform.value.baseTexture;
